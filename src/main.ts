...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* Step icons */
        .timeline-item.pending .timeline-marker::before { content: "⏳"; }
        .timeline-item.in_progress .timeline-marker::before { content: "⚡"; }
        .timeline-item.completed .timeline-marker::before { content: "✅"; }
        .timeline-item.failed .timeline-marker::before { content: "❌"; }
    </style>
</head>
<body>
    <h1>🚀 Pluto App</h1>
    ...
    <script>
        ...
        async function loadSensitiveData() {
            ...
            try {
                ...
                if (result.success && result.data && result.data.length > 0) {
                    let html = '<table style="width: 100%; border-collapse: collapse; margin-top: 10px;">';
                    html += '<tr style="background: #f5f5f5;"><th style="border: 1px solid #ddd; padding: 8px;">Workspace ID</th><th style="border: 1px solid #ddd; padding: 8px;">Token (Masked)</th><th style="border: 1px solid #ddd; padding: 8px;">Created At</th></tr>';
        ...
        function loadTenantDeployments() {
            const dataDiv = document.getElementById('tenantDeploymentsData');
            dataDiv.innerHTML = 'Tenant deployment history viewing is temporarily disabled. The audit data is still being recorded in the database.';
        }
